...
% Description: High-performance data logger with Interactive Toggle Dashboards.
    ...
    elseif ismember(mode, {'plot', 'plot_multi'})
        ...
        if ismember('act', active_plots)
            ...
            % Chỉ vẽ chi tiết Actuator cho Test đầu tiên để tránh rối mắt, 
            % hoặc bạn có thể dùng vòng lặp if is_multi tương tự Position.
            h_data = histories{1}; 
        ...
        if ismember('servo', active_plots)
            tabServo = uitab('Parent', tgroup, 'Title', '4. Servo Analysis');
            h_data = histories{1};
            ...
            title(ax1, 'Biên độ góc xoay Servo \alpha', 'FontWeight', 'bold');
            ...
            title(ax2, 'Vận tốc xoay Servo d\alpha/dt', 'FontWeight', 'bold');
            ...
            title(ax3, 'Chênh lệch lực đẩy Trên-Dưới (\Delta Thrust)', 'FontWeight', 'bold');
